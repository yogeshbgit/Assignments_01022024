...
public class irctcPageObject {
	...
	public void closeLoginPopup() throws InterruptedException
	{
		...
		wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector("a[class='fa fa-window-close pull-right loginCloseBtn ng-tns-c19-13']")));
		loginPopup.click();;
	}
	...
	{	
		...
		Thread.sleep(1000);
		// Click on Selected Source
		for(int i=0;i<Options.size();i++)
		{
			
			//System.out.println("source:"+Options.get(i).getText());
	...
	public void  enterDestination(String destination) throws InterruptedException
	{
		...
		Thread.sleep(1000);
		// Click on Selected Destination
		for(int i=0;i<Options.size();i++)
		{
			
			//System.out.println("Destination:"+Options.get(i).getText());
	...
	public int getNoOfTrainRuuning()
	{
		String noOfTrains = noOfTrainRunning.getText().split(" Result")[0];
